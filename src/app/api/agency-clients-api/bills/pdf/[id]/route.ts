...
export async function GET(req: Request) {
  try {
    ...
    // Minimal fix: cast session.role to string for this check to avoid TS error
    if (
      !session ||
      ((session.role as string) !== "CLIENT" && (session.role as string) !== "AGENCY_CLIENT")
    ) {
    ...
    // Authorize: check bill belongs to logged-in client
