...
function formatAddressTwoLines(
  area?: string,
  city?: string,
  state?: string,
  country?: string,
  ...
  try {
    ...
    const bill = await prisma.bill.findUnique({
      ...
      include: {
        client: true,
        agency: true,
        items: { include: { ad: true, device: true } },
      },
    });
    ...
    const doc = new PDFKit({ size: "A4", margin: 50, font: fontPath });
    ...
    // Line
    doc.strokeColor("#cbd5e0").lineWidth(1).moveTo(leftX, doc.y).lineTo(doc.page.width - doc.page.margins.right, doc.y).stroke();
    doc.moveDown(0.5);
    ...
    y += 10;
    doc.moveDown(0.5);
    doc.strokeColor("#cbd5e0").lineWidth(1).moveTo(startX, y).lineTo(startX + totalTableWidth, y).stroke();
    y += 10;
